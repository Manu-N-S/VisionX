...
def check_for_class(text):
    default = -1  # Default class label when no match is found
    if text == "":
        return default
    classes = {
        "person": 0,
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        print(type(xyxy[0]))
                        print(type(label))
                        xyxy_float = [tensor.float() for tensor in xyxy]
                        cx = int(xyxy_float[0] + (xyxy_float[2]-xyxy_float[0])/2)
                        cy = int(xyxy_float[1]+ (xyxy_float[3]-xyxy_float[1])/2)
                        action=""
                        if xyxy is not True:
                            action = "PLEASE MOVE"
                        if cx<300:
                        ...
                        if cx>340:
                            action="RIGHT"
                        if cx <=340 and cx>=300 and xyxy_float[1] <420:
                            action="MOVE FORWARD"
                        if cx<=340 and cx>=300 and xyxy_float[1] >= 420:
                            action="Stretch your hand to pick the object"
                        return action
                        print("\n")
...
@app.route('/objfind',methods=['post'])
def get_coordinates():
    data = request.json
    print(data)
    ...
    #classno = check_for_class(text)
    action=run()
    os.remove("img.jpg")
    return jsonify({'output': action}), 200
